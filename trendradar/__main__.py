...
class NewsAnalyzer:
    ...
    def _get_mode_strategy(self) -> Dict:
        ...
        if self.report_mode == "daily" and self.report_window_hours > 0:
            ...
            strategy["description"] = (
                f"历史日汇总模式（北京时间 {report_date} 00:00-23:59）"
            )
        return strategy
    ...
    def _load_analysis_data(
        ...
        try:
            ...
            all_results, id_to_name, title_info = self.ctx.read_today_titles(
                current_platform_ids, quiet=quiet, date=date
            )

            if not all_results:
                print(f"没有找到 {date or '当天'} 的数据")
                return None
            ...
            # 历史日报不把今天刚抓取的数据标记为目标日期的“新增”。
            if date and date != self.ctx.format_date():
                new_titles = {}
            else:
    ...
    def _execute_mode_strategy(
        ...
        # 调度系统
        scheduler = self.ctx.create_scheduler()
        schedule = schedule or scheduler.resolve()
